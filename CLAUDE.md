...
The plugin is deliberately narrow: three sensors per gateway (production, consumption, grid), nothing else, plus an opt-in fourth for grid export. **Every endpoint is one-directional** — production exports, consumption and grid import, grid export exports — so none of them ever publishes a negative power or declares two directions. That is the shape the Home app has always handled correctly. Every published number is the gateway's own — no offsets, no generations, no arithmetic to hide a value the gateway reports. It registers **no HomeKit/HAP accessories** — HAP has no power or energy characteristic, so it cannot drive the Energy view.
...
npm test             # node:test suite in test/, drives the real client against a fake gateway
```
...
- Grid: `activePower` (positive when drawing, 0 when exporting) + `cumulativeEnergyImported`
- Grid export: `activePower` (positive when pushing, 0 when importing) + `cumulativeEnergyExported`, opt-in via `gridExportSensor`
- **No endpoint declares both directions.** One that did was the default until v1.15.0 and it was never reliable: the August 2026 iOS 27 build read only its exported half and ignored import; by September it read both but appeared to display their difference; and massive bars in both directions kept arriving on a clean install with zeroed counters. Every endpoint is now single-direction, matching production and consumption, which have never misbehaved. Do not reintroduce a two-directional endpoint.
- Sensor names are **fixed and bare** — `Solar`, `Consumption`, `Grid`, `Grid Export` (`SensorNames` in constants.js). There is no name option, because Homebridge re-asserts each accessory's node label on every start, so a controller may overwrite whatever the user chose and revert to the plugin's; a short default makes that harmless where a long prefixed one made it irritating. Users rename in the Home app.
...
- **An unknown total is never published as zero** (`hasEnergy()`): a zero followed by the real value charts the sensor's whole lifetime as one hour (observed: ~42 MWh of solar in one bar). `update()` sends power only when the reading has no total, and startup (`readUntilReady()`) does not register until `/production.json` has answered and every published sensor's total is known — registering from a failed first read also left Consumption and Grid out until the next restart.
...
- Lifetime energy is additionally **pinned to the physical register it was first read from** (`pinEnergySource()`), as a backstop for the switches the selection rule cannot prevent — `/api/v1/production` standing in for `/production.json`, chiefly. Within a source, the entry's own total is locked to once it has ever been seen; a reading offering only the sum of its `lines`, and the `/api/v1/production` stand-in (source `fallback`), are **provisional and never pin on their own** — they are held (`energyLifetime: null`, floor holds) for up to `PROVISIONAL_PIN_AFTER` polls, because a transient omission of the total, or a failed `/production.json`, on the first poll would otherwise fasten production to the wrong register and reject the real one for the life of the run. Only a gateway that offers nothing better settles on one. The stand-in turning up after the pin is routine and held silently. A gateway offers production's lifetime from four different counters — the CT entry's own `whLifetime`, the sum of that entry's `lines`, the microinverters' entry, and `/api/v1/production` — with four different values, and the plugin chooses between them per reading (by `activeCount`, and by which fields are present). Switching mid-run steps a monotonic counter, and the high-water floor does not catch it because it clamps decreases while a switch upward is an increase. That was survivable while each sensor published only its own register; since grid energy became `consumption - production` it lands in the grid counters one-directionally, reading as export. Observed live as large overnight export bars. Power is deliberately **not** pinned — it is instantaneous, and switching sources for it is correct.
- Grid energy is **measured, not integrated**, wherever house load is measured: `netEnergy = consumption.energyLifetime - production.energyLifetime`, differenced per poll and sorted by sign (`GridEnergy.measure()`). Both registers are floored to a high-water mark *before* `readGrid()` sees them, because a dip in either would read as flow that never happened and inflate both counters. But a poll is **only sampled when both registers were read fresh** this poll (not held by the pin or floor, not missing because `/production.json` failed); otherwise it is skipped outright — no integration fallback, no held value. The registers keep counting, so the next fresh poll carries the whole gap. Sampling a held register credited phantom import that the plausibility guard then refused to take back. Integrating `wNow` is the fallback for gateways without a `total-consumption` CT only (`GridEnergy.integrate()`), and it is strictly worse: `wNow` is unbounded, and one transient reading of a few hundred kW fabricates tens of kWh — observed live as a 23 kWh "export" at 5 a.m. Do not route energy through power again. The two paths keep **separate clocks** (`lastAt` vs `lastNetAt`); the integrated one must not advance its clock on an unreadable sample, or the energy either side of the gap is silently dropped. `lastNet` **and** `lastNetAt` are persisted, so the 25 kW plausibility guard applies to the first increment after a restart too — a register that reset while the plugin was down is refused rather than credited, while a genuine long gap still passes because hours of elapsed time make any real increment imply ordinary power.
- Grid counters are **gross-directional**: each interval is split by sign, so `imported` and `exported` mean the same thing the Enphase app's daily Imported and Exported mean. Neither is net.
- **The daily summary reports the registers the counters came from**, and that line is the diagnostic of record for the grid sensor. The counters are derived from production and consumption, so the two accounts must agree: a day's net must equal the change in `consumption - production`, and neither counter can exceed what the registers moved. When import and export together far exceed that net, flow is being attributed in both directions that never crossed the meter — invisible in the counters alone. `reportDailyGrid()` prints both and names the disagreement. **Read that line before theorising about a chart.**
