...
- `periodicEnergyTest` (default off) publishes one extra grid sensor declaring **periodic energy only** — no cumulative counter beside it, so whether the Home app populates it is an unambiguous answer. Periodic carries `startTimestamp` **and** `endTimestamp`, the inverse of the cumulative rule: the cluster spec forbids `startTimestamp` on cumulative and requires it on periodic (Matter 1.6 Cluster § 2.12.5.2.2-3). Its value is a difference against what that sensor last *published*, so the anchor advances inside the publish gate, never per poll. The step detector and the opening-value notice both skip periodic fields: a periodic value is already a difference, so falling is ordinary and there is no opening bar.
